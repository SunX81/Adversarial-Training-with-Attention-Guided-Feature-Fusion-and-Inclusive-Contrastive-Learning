...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.crop_residual_block = nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)


    def attn_fusion(self, x, pro, attn):
        """Attention-Guided Feature Fusion"""
        # Calculate the attention difference
        B = int(attn.shape[0] / 2)
        attn = torch.mean(attn, dim=1)
        attn = attn / attn.sum(dim=-1).unsqueeze(-1)
        attn = attn[:, 0, 1:]
        attn_clean, attn_adv = torch.split(attn, [B, B], dim=0)
        diff_attn = attn_clean - attn_adv

        # Attention difference sorting
        prop_adv = torch.sort(diff_attn, 1)
        min = prop_adv.indices[:, :pro]
        max = prop_adv.indices[:, -pro:]

        # piecewise feature fusion
        x_clean, x_adv = torch.split(x, [B, B], dim=0)
        x_mix = torch.zeros(x_adv.size()).cuda()
        for i, v in enumerate(min):
            for ind in v:
                x_mix[i, int(ind) + 1] = x_clean[i, int(ind) + 1]

        for i, v in enumerate(max):
            for ind in v:
                x_mix[i, int(ind) + 1] = x_clean[i, int(ind) + 1]
        x_fus = torch.where(x_mix == 0, x_adv, x_mix)

        x_new = torch.cat([x_clean, x_fus], dim=0)
...
class VisionTransformer(nn.Module):
    ...
    def build_2d_sincos_position_embedding(self, temperature=10000.):
        ...
        num_token = 1   # 2 if self.dis_token is not None else 1
...
class AFICL(nn.Module):
    """backbone + projection head"""
    def __init__(self, name='vit-b-16', num_classes=1000,  mlp_hidden=4096, out_dim=256):
        super(AFICL, self).__init__()
        ...
        self.projector = nn.Sequential(
            nn.Linear(dim_in, self.mlp_hidden),
